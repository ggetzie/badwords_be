...
func (app *application) routes() http.Handler {
	...
	// Puzzle Routes
	router.HandlerFunc(http.MethodGet, "/v1/puzzles", app.requirePermission(data.PuzzlesRead, app.listPuzzlesHandler))
	router.HandlerFunc(http.MethodPost, "/v1/puzzles", app.requirePermission(data.PuzzlesCreate, app.createPuzzleHandler))
	router.HandlerFunc(http.MethodGet, "/v1/puzzles/:id", app.requirePermission(data.PuzzlesRead, app.getPuzzleByIdHandler))
